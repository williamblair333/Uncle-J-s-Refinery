...
LIMIT="${2:-}"
LOG="$REPO/state/memweave-sync.log"
LOCK="/tmp/memweave-sync.lock"

mkdir -p "$REPO/state"

if [ ! -x "$VENV" ]; then
  echo "[$(date -Iseconds)] sync ERROR — memweave venv missing at $VENV" | tee -a "$LOG" >&2
  exit 1
fi
...
exec 200>"$LOCK"
if ! flock -n 200; then
  echo "[$(date -Iseconds)] sync skipped — another sync holds $LOCK" | tee -a "$LOG"
  exit 0
fi
