...
- **Phase 2 — Accuracy instrumentation** (IN PROGRESS — Tasks 1–4 + 2.5 + 2.6 done; recall harness live with 25 distinct-drawer probes; diverse `chroma-baseline` recall@5=**0.0** / vector_failure_rate=0.28 — top-k chunks monopolized by mega-files, strong Task 9 evidence; **Task 2.7** next = metric resolution, dedup retrieved to distinct drawers / raise k; Tasks 5–9 remain) — memory recall benchmark (ground-truth probes, weekly cron, ChromaDB baseline), correction ledger hook, citation audit, usage counters for dreaming/ralph/telegram (audit gap); then **backend selection**: turbovecdb / sqlite-vec / roll-our-own / keep — nothing sacred incl. MemPalace app layer; winner triggers D2
